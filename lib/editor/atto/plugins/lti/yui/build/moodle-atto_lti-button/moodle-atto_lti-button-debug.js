...
Y.namespace('M.atto_lti').EmbeddedContentRenderingStrategy = function (item,
    ...
    var TEMPLATES = {
        ...
        video: Y.Handlebars.compile('<video src="{{url}}"'
                + '{{#if width}}width="{{width}}" {{/if}}'
                + '{{#if height}}height="{{height}}" {{/if}}'
                + '{{#if placementAdvice.width}}width="{{width}}" {{/if}}'
                + '{{#if placementAdvice.height}}height="{{height}}" {{/if}}'
                + '></video>'
                ),
        link: Y.Handlebars.compile('<a href="/lib/editor/atto/plugins/lti/view.php?custom={{custom}}&'
                + 'course={{course.id}}&ltitypeid={{toolid}}&resourcelinkid={{resourcelinkid}}'
                + '{{#if item.url}}&contenturl={{item.url}}{{/if}}'
                + '" '
                ...
                + '{{#if item.thumbnail.width}}width={{item.thumbnail.width}} {{/if}}'
                + '{{#if item.thumbnail.height}}height={{item.thumbnail.height}} {{/if}}'
                + '/> '
                + '{{/if}}'
                + '{{item.title}}'
                + '</a>'
